...
current_goal_file() {
  local f st
  for f in "$TMP/.goal/goals"/*.json; do
    [ -f "$f" ] || continue
    st=$(jq -r '.status // ""' "$f" 2>/dev/null || true)
    case "$st" in
      pursuing|paused|needs-input) printf '%s\n' "$f"; return 0 ;;
    esac
  done
  for f in "$TMP/.goal/goals"/*.json; do
    [ -f "$f" ] || continue
...
[ "$(jq '.audit.checklist | length' "$(current_goal_file)")" -ge 3 ] || fail "default scaffold did not create audit items"
"$GOALCTL" --root "$TMP" clear >/dev/null
"$GOALCTL" --root "$TMP" create --quick "Quick objective" >/dev/null
[ "$(jq -r '.audit == null' "$(current_goal_file)")" = "true" ] || fail "--quick should skip audit scaffold"
...
[ "$(jq '.audit.checklist | length' "$(current_goal_file)")" -eq 3 ] || fail "template audit count"
[ "$(jq -r '.budget.kind' "$(current_goal_file)")" = "tokens" ] || fail "template budget"
[ "$(jq -r '.roles.build' "$(current_goal_file)")" = "codex" ] || fail "template roles"
...
STATE_FILE=$(current_goal_file)
jq '.status="pursuing" | .time_used_seconds=0 | .observed_at=$now | .active_turn_started_at=$now' --arg now "$NOW" "$STATE_FILE" > "$STATE_FILE.tmp" && mv "$STATE_FILE.tmp" "$STATE_FILE"
rpc report_progress '{"audit_item_id":"a1","status":"passed","evidence_ref":"v3-smoke"}' >/tmp/v3-rpc
[ "$(jq -r '.audit.checklist[] | select(.id=="a1") | .status' "$(current_goal_file)")" = "passed" ] || fail "report_progress did not mark audit"
...
[ "$(jq -r '.status' "$(current_goal_file)")" = "paused" ] || fail "report_stuck attempts=5 should pause"

step "5. Watch, history, debrief, PR, notifier/webhook, sync"
STATE_FILE=$(current_goal_file)
jq '.status="achieved" | .time_used_seconds_final=420 | .tokens_used_final=47000' "$STATE_FILE" > "$STATE_FILE.tmp" && mv "$STATE_FILE.tmp" "$STATE_FILE"
STATE_GID=$(jq -r '.goal_id' "$STATE_FILE")
GOAL_ID="$STATE_GID" "$GOALCTL" --root "$TMP" watch --once >/tmp/v3-watch
...
GOAL_ID="$STATE_GID" "$GOALCTL" --root "$TMP" history archive --retry >/tmp/v3-archive
"$GOALCTL" --root "$TMP" history list >/tmp/v3-history
grep -q 'Fix login bug' /tmp/v3-history || fail "history list missing archived goal"
GOAL_ID="$STATE_GID" "$GOALCTL" --root "$TMP" debrief >/tmp/v3-debrief
grep -q 'Goal Debrief' /tmp/v3-debrief || fail "debrief missing"
GOAL_ID="$STATE_GID" "$GOALCTL" --root "$TMP" pr --json >/tmp/v3-pr
...
GOAL_ID="$STATE_GID" node "$ROOT/bin/goal-v3" notify-webhook "$TMP" 0 achieved '{"type":"goal.achieved"}'
...
compgen -G "$TMP/.goal/goals/*.json" >/dev/null || fail "sync pull did not restore goal records"
